...
farm_names = list(koepke="Koepke", pagel="Pagel", saxon="Saxon", df1="DF1", df3="DF3", df5="DF5", df7="DF7", df8="DF8")
@
...
<<label=SM_breakpoints, echo=FALSE, include=FALSE, keep.source=True>>=
xs = seq(33, 43)
...
<<label=I30_by_binned_SM, echo=False>>=
    ...
    for(site in farms)
    {
        limits[[site]] = c(0, breakpoints[[site]]-5, breakpoints[[site]], Inf)
...
<<label=binned_I30_plots, include=False, echo=False>>=
    ...
    for(site in farms)
    {
        titles = c(paste(farm_names[[site]], " (SM < ", limits[[site]][2], "%)", sep=""), paste(farm_names[[site]], " (", limits[[site]][2], "% <= SM < ",
                     limits[[site]][3], "%)", sep=""), paste(farm_names[[site]], " (", limits[[site]][3], "% <= SM)", sep=""))
        site_data = data[data$site==site,]
...
<<label=binned_I30_plots2, include=False, echo=False>>=
    ...
    for(site in farms)
    {
        titles = c(paste(farm_names[[site]], " (SM < ", limits2[[site]][2], "%)", sep=""), paste(farm_names[[site]], " (", limits2[[site]][2], "% <= SM)", sep=""))
        site_data = data[data$site==site,]
